...
class Mapper:
    ...
    def map_foam_case_to_dto(cls, foam_case: FoamCase) -> FoamCaseDTO:
        cam_prop_dtos = [cls.map_camera_props_to_dto(cam_prop) for cam_prop in foam_case.cam_prop_list]
        slice_dtos = []
        ...
        return FoamCaseDTO(cases_path, cam_prop_dtos, slice_dtos, foam_case.name, _id=foam_case.idn)
    ...
    def map_camera_props_to_dto(cls, c: CameraProps) -> CameraPropsDTO:
        ...
        return CameraPropsDTO(focal_point_dto, cam_position_dto, c.viewangle, viewup, c.name, _id=c.idn)
